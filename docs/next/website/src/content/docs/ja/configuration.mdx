...
トークンのテーブルでは、色やスタイルとは独立した `separator_before` も指定できます。`" "` は空白 1 個、`""` は直前の表示トークンとの直接連結を表します。省略時は従来どおり、`state_icon` の後または `git_status` の前は空白、それ以外は ` · ` です。先行トークンが未設定、ルールで非表示、または幅不足で省略された場合も、最初の表示トークンの前には区切りを表示しません。Space 行、Agent 行、`rows_by_agent` で使用できます。区切りに含まれる制御文字は無視されます。アイコンと `git_status` は常に表示対象のため、サイドバーが狭いと長い区切りに押し出されて端で切れることがあります。
