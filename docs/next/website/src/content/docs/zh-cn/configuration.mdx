...
令牌表还支持独立于颜色和样式的 `separator_before`。设为 `" "` 表示一个空格，设为 `""` 则直接连接到前一个可见令牌。省略时保留原有规则：`state_icon` 之后或 `git_status` 之前使用空格，其余位置使用 ` · `。即使前面的令牌缺失、被规则隐藏或因宽度不足而省略，第一个可见令牌之前也不会显示分隔符。Space 行、Agent 行和 `rows_by_agent` 均支持此选项。分隔符中的控制字符会被忽略。图标和 `git_status` 始终保留，因此在较窄的侧边栏中，过长的分隔符可能把它们挤出边缘而被截断。
